...
const Ciclo = () => {
  ...
  return <div className="pb-24">
      <Alert className="mx-4 mt-4 bg-lavanda-50 border-lavanda-200">
        ...
        <AlertDescription className="text-lavanda-700">Selecione o primeiro dia da sua menstruação no calendário e clique em "Inicio do Período"</AlertDescription>
      </Alert>
      <div className="bg-gradient-sunset pt-6 pb-6 px-4 rounded-b-[30px]">
        <CycleTabs activeTab={activeTab} setActiveTab={setActiveTab} cycles={cycles} currentPhase={currentPhase} daysUntilNextPeriod={daysUntilNextPeriod} averageCycleLength={averageCycleLength} lastPeriodDuration={lastPeriodDuration} symptoms={symptoms} onCycleCalculated={() => {}} onAddSymptoms={() => handleAddSymptoms()} />
      </div>
      {/* Diálogo de registro de sintomas */}
      <AddSymptomsDialog open={dialogOpen} onOpenChange={handleDialogChange} date={dialogDate} />
      <BottomNavigation />
    </div>;
};
